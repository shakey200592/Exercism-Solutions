...
def approx_average_is_average(hand):
    ...
    approx_card_average = card_average([hand[0], hand[-1]])
    middle_index = len(hand) // 2

    return float(approx_card_average) == card_average(hand) or float(
        hand[middle_index]
    ) == card_average(hand)
...
def average_even_is_average_odd(hand):
    ...
    """
    even_indexed = [card for index, card in enumerate(hand) if index % 2 == 0]
    odd_indexed = [card for index, card in enumerate(hand) if index % 2 != 0]

    return float(card_average(even_indexed)) == card_average(odd_indexed)
